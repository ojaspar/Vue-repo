...
export default class SalesValidationMixin extends Mixins(UtilityMixin) {
    ...
    public validateDateReceived(dateReceived: string) {
        ...
        if (!isEmpty(this.updatePaymentFormError['dateReceived'])) return;

        if (!this.isDateOnOrBeforeToday(dateReceived)) {
            this.updatePaymentFormError['dateReceived'] = 'You cannot record a payment that has not happened';
            return;
        }

        this.updatePaymentFormError['dateReceived'] = '';
        return;
    }
