    ...
    <div>
        ...
        <div class="large-donor-transaction-table">
            <b-row class="donor-transaction-table__heading">
                <b-col class="pl-0">Form of Donation</b-col>
                <b-col>Donor</b-col>
            ...
            <div v-if="donationsAreAvailable">
                <b-row
                    ...
                    :key="index"
                >
                    <font-awesome-icon icon="ellipsis-h" class="pointer mr-4" />
                    ...
                    <b-col>
                        <p class="content-text m-0">{{ transaction.donorName || 'Donor name recored' }}</p>
                    </b-col>
                    <b-col cols="3">
                        <p class="content-text m-0">{{ transaction.description || 'None' }}</p>
                    </b-col>
                    <b-col>
                        <p class="content-text m-0" v-if="transaction.currentAmount.amount > 0">
                            {{ transaction.currentAmount | amountWithCurrency }}
                        </p>
                ...
                >
                    ...
                    >
                        <div class="product-transaction-mobile-table__content--details">
                            ...
                            <p class="mobile-content__text" v-if="transaction.currentAmount.amount > 0">
                                {{ transaction.currentAmount | amountWithCurrency }}
                            </p>
...
import { FormOfDonation, RedemptionTypeOption, SchemeDonation } from '@/pages/Donations/types';
import { VueExtension } from '@/types';

const defaultTransaction: SchemeDonation = {
    donorName: 'Alaanu Samaria',
    transactionDate: '2020-11-20T00:00:00+00:00',
    currentAmount: { currencyKey: 0, amount: 0 },
    pledgeDueDate: '',
    donationScheme: 'End SARS',
    donationKey: '',
    redemptionType: RedemptionTypeOption.PLEDGE,
    description: null,
    donationForm: FormOfDonation.FINANCIAL,
    targeValue: { currencyKey: 0, amount: 0 },
};
...
export default class DonationSchemeTransactionTable extends Vue {
    @Prop({ default: () => defaultTransaction }) readonly transactions!: SchemeDonation[];
...
<style lang="scss">
@import 'src/styles/pages/donations/donation-transaction-table';
</style>
